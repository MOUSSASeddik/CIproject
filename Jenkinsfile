pipeline {
    ...
    stages {
	 ...
	 stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user jenkinsadmin:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' '192.168.204.129:8080/job/Reddit-Clone-CD/buildWithParameters?token=gitops-token'"
                }
            }
         }
     }
